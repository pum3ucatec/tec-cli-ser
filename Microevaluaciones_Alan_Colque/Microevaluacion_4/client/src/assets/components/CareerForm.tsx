...
interface Props { initial?: Omit<Career, 'ID'>; onSubmit: (data: any) => void; }

export default function CareerForm({ initial, onSubmit }: Props) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="flex flex-col gap-4 bg-white p-6 rounded shadow">
      <div>
        <label className="block text-sm font-bold text-gray-700">Nombre Carrera</label>
      ...
      <button className="bg-orange-500 text-white p-2 rounded font-bold hover:bg-orange-600 transition">Guardar Carrera</button>
    </form>
  );
}